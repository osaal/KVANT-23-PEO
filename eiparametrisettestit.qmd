...
\end{split}
$$ {#eq-w-keskiarvo-ja-keskihajonta}
...
Keskihajonta $\text{SE}_{\bar{W}}$ lasketaan kertomalla molempien ryhmien otoskoot yhteen, kertomalla tämä tulos molempien ryhmien otoskokojen summalla plus yksi, jakamalla tulos 12:lla, ja ottamalla viimeisen tuloksen neliöjuuri.
...
Z-arvon laskeminen ja p-arvon hakeminen on täysin sama kuin Wilcoxonin testissä (ks. @sec-wilcoxonin-testin-toteuttaminen-excelissä).
...
[Merkitsevyyden laskemiseksi]{.underline} voidaan käyttää joko **z-normalisointia**, jolloin pitää myös ensin tehdä Fisherin muuntolasku, tai **t-suuretta**.

[Jos haluat laskea z-normalisoidun rho-suureen]{.underline}, käytä seuraavia kaavoja:
...
Z-suureen kaavassa lasket ensin otoskoon $n$ minus kolme, ja jaat tulokset $1.06$:lla. Otat sitten tämän tuloksen neliöjuuren.

Sitten sinun tulee laskea Fisherin muuntolasku korrelaatiokertoimelle, $F(r)$. Jaa ensin rho-kerroin plus yksi laskulla rho-kerroin minus yksi. Ota tuloksen luonnollinen logaritmi, ja kerro tämä tulos puolella ($0.5$).

Viimeiseksi, kerro Fisherin muuntolasku aikaisemmin lasketulla neliöjuurella. Saat z-normalisoidun arvon, jolle voit hakea todennäköisyysarvon $p$ yleisellä tavalla (kriittisten arvojen taulukosta tai laskurilla).

[Jos haluat käyttää t-suuretta]{.underline}, käytä seuraavaa kaavaa:

$$
t = r\sqrt{\frac{n-2}{1-r^2}}
$$

T-suureen kaava on paljon yksinkertaisempi. Laske ensin t-jakauman vapausasteet $n-2$, sitten rho-suureen neliö $r^2$ ja sen inverssi $1-r^2$. Jaa vapausasteet tällä inverssillä, ja ota tuloksen neliöjuuri. Kerro viimeiseksi neliöjuuren tulos rho-suurella $r$. Lopputulos on t-suure, joka seuraa Studentin t-jakaumaa vapausasteilla $n-2$. Voit hakea todennäköisyysarvon t-suureelle (kaksihäntäisen) kriittisten arvojen taulukolla tai laskurilla.

### Spearmanin rhon laskeminen Excelissä
...
1.  Laske molempien muuttujien järjestysarvot: `=ARVON.MUKAAN.KESKIARVO(solu; alue; 1)`.
2.  Laske järjestyslukualueiden korrelaatiokerroin: `=KORRELAATIO(muuttuja1; muuttuja2)`. Huomaa, että alueina pitää olla järjestysluvut, ei alkuperäiset luvut!
3.  Valitse merkitsevyyslaskentatapa ja syötä siihen tarvittavat numerot.
    1.  Jos z-arvo:
        1.  Laske Fisherin muuntolasku käyttämällä aikaisempaa korrelaatiokertointa sekä luonnollisen logaritmin funktiota `LUONN.LOG()`.
        2.  Laske sitten z-arvo käyttäen Fisherin muuntolaskun tulosta ja otoskokoa (hae esim. `LASKE`-funktiolla).
        3.  Hae viimeiseksi z-arvon merkitsevyys funktiolla `=NORM_JAKAUMA.NORMITA(z; EPÄTOSI)`.
    2.  Jos t-arvo:
        1.  Laske t-arvo käyttämällä otoskokoa (hae esim. `LASKE`-funktiolla) ja aikaisempaa korrelaatiokertointa. Neliöjuuren lasket funktiolla `NELIÖJUURI()`.
        2.  Hae sitten t-arvon merkitsevyys funktiolla `=T.JAKAUMA(t; vapausasteet; EPÄTOSI`). Vapausasteet ovat $n-2$.

### Spearmanin rhon raportointi

Raportoi rho-testistä mieluisesti seuraavat:

-   Spearmanin testisuure $\rho$ (vaihtoehtoisesti kirjoitettuna $r_s$)

-   Z- tai t-koko $z$ tai $t$

    -   Jos $t$: Vapausasteet $\text{df}$
